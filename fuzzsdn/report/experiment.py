...
from fuzzsdn.common.metrics import density, fraction_of_borderline_points, geometric_diversity, imbalance_ratio, standard_deviation
from fuzzsdn.common.utils import terminal
from fuzzsdn.common.utils.terminal import progress_bar
from fuzzsdn.app.experiment import Model
...
def fetch_experiment_data(hostname, port, username, password, experiment, ignore_existing=False, quiet=False):
    ...
    remote_exp_dir = os.path.join(usr_home, ".local/share/fuzzsdn/experiments")
...
def generate_graphics(experiment, test_data : Optional[str] = None, display: bool = False):
    ...
    if not test_data:
        if "has_rule_gen_info" in expt_info and expt_info["context"]["has_rule_gen_info"] is True:
            avg_gen_acc = []
...
def generate_report(experiment : str):
    ...
    for i in range(expt_info["context"]["iterations"]):
        ...
        if rules[i] is not None:
            for rule in rules[i]:
                nb_of_rules += 1
                if "has_rule_gen_info" in expt_info["context"]:
                    conditions = (
...
def generate_result_csv(experiment : str, test_data : Optional[str] = None):
    ...
    df['accuracy']     = expt_info['learning']["accuracy"][:it_cnt]
    df['precision']    = expt_info['learning'][tgt_cls]["precision"][:it_cnt]
    df['recall']       = expt_info['learning'][tgt_cls]["recall"][:it_cnt]
    df['mcc']          = expt_info['learning'][tgt_cls]["mcc"][:it_cnt]
    df['auroc']        = expt_info['learning'][tgt_cls]["auroc"][:it_cnt]
    df['auprc']        = expt_info['learning'][tgt_cls]["auprc"][:it_cnt]

    # Add the data for the tests
    if 'test_evl' in expt_info and test_data:
        df['test_accuracy']     = expt_info['test_evl']["accuracy"][:it_cnt]
        df['test_precision']    = expt_info['test_evl'][tgt_cls]["precision"][:it_cnt]
        df['test_recall']       = expt_info['test_evl'][tgt_cls]["recall"][:it_cnt]
        df['test_mcc']          = expt_info['test_evl'][tgt_cls]["mcc"][:it_cnt]
        df['test_auroc']        = expt_info['test_evl'][tgt_cls]["auroc"][:it_cnt]
        df['test_auprc']        = expt_info['test_evl'][tgt_cls]["auprc"][:it_cnt]

    df['data_imbalance']        = expt_info['data']["imbalance"]
    if 'geometric_diversity' in expt_info['data']:
        df['data_geom_div']         = expt_info['data']["geometric_diversity"][:it_cnt]
